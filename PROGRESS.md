...
## 2026-08-10T18:34:50Z — Tier 2: the double-entry schema, the atomic conditional debit, and G1's first form

> Concurrent-session note: this entry covers `packages/ledger/**`, `tooling/db/**`,
> `tests/integration/ledger-schema.test.ts`, and appended `ASSUMPTIONS.md` sections. There is an
> unresolved ownership disagreement with the other session over `packages/ledger`; see the closing
> paragraph. It does not affect any result recorded here.

### Behaviour delivered

- **`packages/ledger/migrations/0001_double_entry_ledger.sql`** — the authoritative monetary schema
  in a dedicated `ledger` schema, applied to the live local PostgreSQL 16.14 as the owner role.
  Every invariant that can be a constraint is a constraint, and every constraint is explicitly
  named so an error mapping keys off a chosen name rather than an autogenerated one:
  - `budgets_available_never_negative` is **I2** enforced by PostgreSQL. Its scope is documented in
    the migration itself: it holds over one row, and it is the invariant only because all three
    columns move together in a single-row `UPDATE`. Sharding the hot row for throughput would
    silently demote it to a per-row sanity check, so that comment names the requirement to replace
    it with something of equal strength first.
  - `reservations_status_shape` is **I3 and I4** as one `CASE`-per-status check: non-terminal rows
    have settled and released nothing, terminal rows partition the reserved amount into exactly one
    settled part and one released part that sum back to it. An imbalanced terminal row cannot exist.
  - `ledger.entries` carries both a debit and a credit account on one row with
    `CHECK (debit <> credit)`. This is a deliberate deviation from textbook double entry, noted in
    the file: two rows would need a sum-to-zero constraint over a group, which PostgreSQL cannot
    express declaratively and which becomes a trigger or application discipline. One row removes
    the failure mode instead — there is no way to write half a movement.
  - Composite foreign keys `(account_id, tenant_id)` mean a cross-tenant entry is unrepresentable at
    the storage layer, underneath RLS rather than instead of it. **I7.**
  - `price_book_version` is `NOT NULL REFERENCES ... ON DELETE RESTRICT`. **I5**: the version that
    authorized an attempt cannot be deleted out from under it.
  - RLS on all six tenant tables, scoped by `app.tenant_id` through a `STABLE` function with a
    pinned `search_path`. Enabled but deliberately not `FORCE`d, so the owner remains the migration
    and cross-tenant system identity while the runtime role — `NOBYPASSRLS`, no owner-role
    membership — is the only identity services receive.
  - Grants enumerate rather than blanket: no `UPDATE` or `DELETE` on `entries`, `audit_events`, or
    `price_book_versions`, and no `DELETE` anywhere. Append-only enforced by grant is stronger than
    append-only enforced by the code that writes it.
- **`packages/ledger/src/migrate.ts`** — ordered, gapless, checksummed, one transaction per
  migration, serialised by advisory lock, and with no `down` path.
- **`packages/ledger/src/repository.ts`** — the operations. `reserveBudget`'s entire admission
  decision is the `WHERE` clause of one conditional `UPDATE`, so there is no interval between
  deciding and writing for a concurrent request to occupy. Settlement computes the released
  remainder in SQL from the stored reservation rather than accepting it from a caller, and the
  `status = 'dispatched'` predicate makes the update its own idempotency check. Every mutation
  writes its ledger entry in the same transaction. `checkLedgerInvariant` recomputes what the hot
  row claims from the reservations themselves and independently checks that entries sum to zero.
- **`tooling/db/migrate.mjs`** — the operator command, run as the schema owner. The runtime role has
  no `CREATE` privilege, so no request path can alter the schema constraining it.

### Commands and evidence

- `node tooling/db/migrate.mjs` — applied `0001_double_entry_ledger.sql` to
  `127.0.0.1:4165/galuxium_nexus_v2` as `galuxium_nexus_v2_owner`.
- `vitest run tests/integration/ledger-schema.test.ts --maxWorkers=1` — **19 of 19 passed** against
  the real engine, nothing mocked.
- **Release gate G1, first form:** 50 simultaneous `reserveBudget` calls against a budget affording
  exactly 5. Result: exactly 5 `reserved`, 45 `insufficient_budget`, `reserved` equal to the whole
  credited amount, `available` exactly `0`, authorized equal to credited and never above it, and
  `checkLedgerInvariant` holding. **Over-authorization events: 0.** This meets the §8 Epoch 1
  ratchet for peak parallel reservations (50) and over-authorization (0). It is not yet G1 in full:
  no linearizability history checking, no chaos, no crash injection.
- Idempotency: 20 simultaneous requests carrying one key produced exactly one reservation and one
  debit. The loser of the race rolls back its own debit because the debit lives in the same
  transaction as the insert that violates the unique constraint.
- Constraint refusals asserted with SQLSTATE and constraint name: `23514` on
  `budgets_available_never_negative`, `reservations_status_shape`,
  `reservations_expire_after_creation`, `entries_two_distinct_sides`; `23503` on
  `entries_credit_account_is_tenant_scoped` and on deleting a referenced price book version.
- RLS and grants: an unscoped runtime connection sees `0` tenants where a scoped one sees exactly
  its own; the runtime role receives `42501` for `DELETE`/`UPDATE` on `entries`, `DELETE` on
  `reservations`, and `CREATE TABLE` in the schema.
- `tsc -b tsconfig.json`, `eslint packages/ledger tooling/db tests/integration --max-warnings 0`,
  and `prettier --check` on every file in this change — all exited 0.

### Three defects the committed gate caught that hand-verification had not

1. **`CREATE DOMAIN ... NOT NULL`.** `NOT NULL` on a domain makes every column using it
   non-nullable, which broke `entries.price_book_version` — the one column that must be NULL,
   because a funding credit precedes every attempt and has no authorizing price version. A domain
   describes a value space; nullability belongs on the column. Fixed by moving `NOT NULL` to each
   column that needs it.
2. **`set_config(..., true)` outside a transaction.** The third argument is `is_local`, so the
   tenant scope lives and dies with the enclosing transaction. `readBudget`, `markDispatched`, and
   `markUncertain` issued single statements with no transaction, so PostgreSQL accepted the call and
   discarded the value. The scope-verification read inside `applyScope` turned that into a loud
   refusal instead of a query that silently returned zero rows — which is the reason the
   verification exists. Fixed by giving every scoped operation its own transaction.
3. **Concurrent queries on one connection.** `Promise.all` over `accountIdOf` calls issued
   simultaneous queries on a single client; pg serialised them and warned that it will stop doing so
   in 9.0. A connection is a sequential resource. Fixed by awaiting in sequence.

Correcting the first meant editing an already-applied migration, which the runner refuses by design.
The recovery was `DROP SCHEMA ledger CASCADE` on the local database and a re-apply, which is
defensible only because this schema has never existed outside a developer's own cluster. That
boundary is now recorded in `ASSUMPTIONS.md` rather than left as a habit.

### What is now true

The hard core exists and is proven against the real engine. A tenant cannot be over-authorized under
50-way concurrency; the database refuses the state rather than the application avoiding it; an
imbalanced terminal reservation and a cross-tenant entry are both unrepresentable; and an unscoped
connection sees nothing. What does not exist: worst-case quoting from a versioned price book, any
provider adapter, the reaper, linearizability history checking, chaos and crash injection, and every
service-level path. The repository remains **not yet in production**.

### Risks, migration, rollback, blockers, and next selection

- Rollback of this change is a source revert plus `DROP SCHEMA ledger CASCADE` on a local database.
  There is no non-local environment to migrate.
- The `entries_one_per_reservation_kind` unique constraint gives exactly-once settle and release
  entries, but the reaper, fencing enforcement in SQL, and the compensating-overspend write path are
  not yet implemented — only their schema support is.
- `tooling/dependencies/upstream.mjs` is currently unformatted in the shared tree and is the other
  session's file; `prettier --check .` fails on it and on nothing else. Not mine to fix.
- **Ownership disagreement, recorded rather than resolved:** the other session states that
  `packages/ledger/**`, `tooling/db/**`, and this integration test are its work. This session
  authored all of them, and commit `babbe0f` is the pushed record of the Tier 1 half. Both sessions
  run as the same git identity, so authorship is not distinguishable from history alone. Committing
  now removes the work from a contested working tree, which matters more than settling attribution.
- §10.1 next: `packages/pricing` — versioned per-provider, per-model, per-token rates and the
