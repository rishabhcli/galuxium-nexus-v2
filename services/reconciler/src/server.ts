...
const EXPECTED_SERVICE = { name: 'reconciler', port: 4161 } as const;
...
async function createDependencies(logger: Logger): Promise<Dependencies> {
  ...
  const ledger = createLedgerHealthProbe({
    ...
    // Direct, admin, and metrics readiness probes can overlap during the
    // canonical health sweep. Eight remains a hard local upper bound and fits
    // inside the repository PostgreSQL max-connections budget.
  ...
  try {
    ...
    redis.on('error', (error: Error) => {
      logger.warn('reconciler.redis_error', { error });
    });
    await redis.connect();
    ...
    try {
      ...
      logger.error('reconciler.dependency_startup_rollback_failed', { error: cleanupError });
    }
    throw error;
  }
}
...
async function main(): Promise<void> {
  ...
  try {
        ...
        if (request.method === 'GET' && (url.pathname === '/' || url.pathname === '/status')) {
          return {
            body: JSON.stringify({
              ...
              message:
                'The reconciler is healthy; delayed-usage processing is not implemented yet.',
    ...
    try {
      ...
      logger.error('reconciler.service_startup_rollback_failed', { error: cleanupError });
    }
    throw error;
  }
}
